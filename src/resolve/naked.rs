use crate::board_util::{contains_candidates_in_list, get_candidates_from_cell, get_x_y_from_box_and_pos};
...
pub(super) fn naked_tuple_check(mut board: SudokuBoard, candidates: Vec<usize>) -> (SudokuBoard, isize) {
  ...
  for axis_a in 0..9 {
    ...
    if occurrences_row == len as u8 {
      let (aux_board, aux_hit) = clean_candidates_line_naked(board, &candidates, axis_a, LineType::Row);
      board = aux_board;
      hits += aux_hit;
    }
    if occurrences_col == len as u8 {
      let (aux_board, aux_hit) = clean_candidates_line_naked(board, &candidates, axis_a, LineType::Col);
      board = aux_board;
      hits += aux_hit;
    }
...
fn clean_candidates_line_naked(mut board: SudokuBoard, candidates: &Vec<usize>, axis: usize, line_type: LineType) -> (SudokuBoard, isize) {
  let mut hits = 0;
  for axis_b in 0..9 {
